...
if($res and mysql_num_rows($res)>0 and $row=mysql_fetch_row($res)) return ((strlen($row[1])==0)?TRUE:$row[1]); else return FALSE;
}
...
if(!isset($GLOBALS['archives'])) $result=mysql_query('select '.$what.' from '.$GLOBALS['Tu'].' LEFT JOIN '.$GLOBALS['Tp'].' ON '.$GLOBALS['Tu'].'.'.$GLOBALS['dbUserId'].'='.$GLOBALS['Tp'].'.poster_id where '.$GLOBALS['Tp'].'.poster_id IS NULL order by '.$GLOBALS['Tu'].'.'.$GLOBALS['dbUserId'].' '.$sortSql.' '.$GLOBALS['makeLim']);
else $result=mysql_query("select {$what} from {$GLOBALS['Tu']} where num_posts=0 and num_topics=0 order by {$GLOBALS['dbUserId']} {$sortSql} {$GLOBALS['makeLim']}");
if($result) {
$GLOBALS['countRes']=mysql_num_rows($result);
$GLOBALS['result']=$result;
}
...
mysql_query("update {$tbUsers} set {$GLOBALS['dbUserSheme']['user_website'][1]} = '' where {$GLOBALS['dbUserId']} = {$user_id}");
}

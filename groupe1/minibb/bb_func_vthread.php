...
$topicDesc=1;
$topic_reverse="<img src=\"{$main_url}/img/topic_reverse.gif\" border=\"0\" alt=\"\" />&nbsp;";
}
...
if ($logged_admin==1 or $isMod==1) $viewIP=' '.$l_sepr.' IP: '.'<a href="'.$main_url.'/'.$indexphp.'action=viewipuser&amp;postip='.$cols[4].'" target="_blank">'.$cols[4].'</a>';
else $viewIP='';
...
$backToLive="{$l_sepr} <a href=\"JavaScript:confirmLive({$topic})\" onmouseover=\"window.status='{$l_arcBackToLive}'; return true;\" onmouseout=\"window.status=''; return true;\">{$l_arcBackToLive}</a>";
$backToLiveJs=<<<out
...
$stickyTopic.=" / <a href=\"{$main_url}/{$indexphp}action=sticky&amp;forum=$forum&amp;topic=$topic&amp;chstat=2\">{$l_makeSupersticky}</a>";
}

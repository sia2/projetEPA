...
Copyright (C) 2004-2007 Paul Puzyrev, Sergei Larionov. www.minibb.net
Copyright (C) 2011,2012,2014-2015 Paul Puzyrev. www.minibb.com
...
insertArray(array('user_id', 'topic_id', 'email_code'), $Ts);
}
}
return;
}
...
if($action=='ptopic' and defined('PREMOD_MSGPOST')) $post_status=1;
else $post_status=0;
...
if($user_id>0) db_calcTotalUserAmount($user_id, $Tu, $Tt, $Tp, $Taus, $arcId);
}
...
$sub0=explode('<<', $sub0[1]);
$msgg=explode('[FOLLOW_CODE]', trim($sub0[1]));
$msg[$langOrig][0]=$msgg[0];
$msggg=explode('[UNSUBSCRIBE_BY_CODE]', $msgg[1]);
$msg[$langOrig][1]=$msggg[0];
$msg[$langOrig][2]=$msggg[1];
$sub[$langOrig]=$sub0[0];
}
...
$allUsers=array();
$allUsersCodes=array();
if($row=db_simpleSelect(0,$Ts,'user_id,active,email_code','topic_id','=',$topic, '', '', 'user_id', '!=', $user_id)) do if($row[1]==1) {
$allUsers[]=$row[0];
$allUsersCodes[$row[0]]=$row[2];
}
...
$allUsers=array();
if($row=db_simpleSelect(0,$Tu,"{$dbUserId}, {$dbUserSheme['user_email'][1]}, {$dbUserSheme['language'][1]}, {$dbUserAct}")) do $allUsers[$row[0]]=array($row[1], $row[2], $row[3]); while($row=db_simpleSelect(1));
unset($xtr);

foreach($allUsers as $k=>$v){

if($v[2]==1){
//will send only to active accounts
...
$sub0=explode('<<', $sub0[1]);
/* FOLLOW_CODE should be strictly BEFORE [UNSUBSCRIBE_BY_CODE] tag! */
$msgg=explode('[FOLLOW_CODE]', trim($sub0[1]));
$msg[$v[1]][0]=$msgg[0];
$msggg=explode('[UNSUBSCRIBE_BY_CODE]', $msgg[1]);
$msg[$v[1]][1]=$msggg[0];
...
$followCode="{$main_url}/{$indexphp}action=vthread&forum={$forum}&topic={$topic}&goPost={$topic_last_post_id}&user={$k}";
$unsubByCode="{$main_url}/{$indexphp}action=unsubscribe&topic={$topic}&usrid={$k}&code={$allUsersCodes[$k]}";

$subS=$sub[$v[1]];
$msgS=$msg[$v[1]][0].$followCode.$msg[$v[1]][1].$unsubByCode.$msg[$v[1]][2];
...
$subS=$sub[$langOrig];
$msgS=$msg[$langOrig][0]."{$main_url}/{$indexphp}action=vthread&forum={$forum}&topic={$topic}&goPost={$topic_last_post_id}&user=1";

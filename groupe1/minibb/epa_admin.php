...
Copyright (C) 2010-2015 Paul Puzyrev. www.minibb.com
Latest File Update: 2015-Oct-05
*/
define ('INCLUDED776',1);

$unset=array('logged_admin', 'isMod', 'user_id', 'langu', 'includeHeader', 'includeFooter', 'mod_rewrite', 'inss', 'insres', 'cook', 'archives', 'rheader', 'allowHyperlinksProfile', 'set_archive', 'setArchive');
...
$iconList.="<a href=\"JavaScript:paste_strinL('{$file}')\" onmouseover=\"window.status='{$GLOBALS['l_forumIcon']}: {$file}'; return true\"><img src=\"{$GLOBALS['main_url']}/{$GLOBALS['archiveImgFld']}img/forum_icons/{$file}\" alt=\"{$file}\" /></a>&nbsp;&nbsp;";
$ss++;
if ($ss==5) {
$ss=0;
$iconList.="<br />\n";
...
$fast.="<img src=\"{$GLOBALS['main_url']}/{$GLOBALS['archiveImgFld']}img/forum_icons/{$row[4]}\" style=\"width:{$GLOBALS['fIconWidth']}px;height:{$GLOBALS['fIconHeight']}px\" alt=\"{$row[1]}\" title=\"{$row[1]}\" id=\"forum{$row[0]}\" />&nbsp;<b><a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=editforum2&amp;forumID={$row[0]}{$GLOBALS['addArcUrl']}\">{$row[1]}</a></b> [ORDER: {$row[3]}] - <span class=\"txtSm\">{$row[2]}&nbsp;</span>&nbsp;&nbsp; <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=move&amp;where=1&amp;forumID={$row[0]}{$GLOBALS['addArcUrl']}\">&uarr;</a>&nbsp;&nbsp;<a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=move&amp;where=0&amp;forumID={$row[0]}{$GLOBALS['addArcUrl']}\">&darr;</a><br />";
...
$addArcMark='<span class="txtSm">/ <strong>'.$archives[$setArchive][0].'</strong><br /></span>';
...
$l_adminpanel_link="<p><a href=\"{$main_url}/{$bb_admin}{$addArcUrlAdm}\">".$l_adminpanel."</a></p>";
...
$addArcMark='<span class="txtSm">/ <strong>'.$archives[$_POST['set_archive']][0].'</strong></span><br />';
}
$text2=ParseTpl(makeUp('admin_panel'));
break;
...
$warning=$l_completed." ({$row} &mdash; {$numdel})";
$text2=ParseTpl(makeUp('admin_panel'));
break;
...
if(!isset($urlSet) or $urlSet=='') $sortOptions='';
else{
if(!isset($l_asc)) $l_asc='&uarr;';
if(!isset($l_desc)) $l_desc='&darr;';
if($sort==0) $sortOptions="<b>{$l_asc}</b>&nbsp;&nbsp;<a href=\"{$urlSet}&amp;sort=1\" style=\"text-decoration:none\">{$l_desc}</a>";
else $sortOptions="<a href=\"{$urlSet}&amp;sort=0\" style=\"text-decoration:none\">{$l_asc}</a>&nbsp;&nbsp;<b>{$l_desc}</b>";
}
...
$correctErr="<a href=\"{$indexphp}action=vthread&amp;topic={$bTopic}#newreply\">$l_back</a>";

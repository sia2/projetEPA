...
if($addDir=='') {
if(defined('MOBILE') and substr($name,0,5)=='email') $addDir=$GLOBALS['pathToFiles'].'templates/';
...
if(substr($dateRF, 0, 10)==$GLOBALS['today']) $dateR=$GLOBALS['l_today'].' '.date($GLOBALS['timeOnlyFormat'], $dfval);
elseif(substr($dateRF, 0, 10)==$GLOBALS['yesterday']) $dateR=$GLOBALS['l_yesterday'].' '.date($GLOBALS['timeOnlyFormat'], $dfval);
else {
...
$c='';
if(!defined('MOBILE')) $ncss='navCell';
else {
if($pars[4]) $ncss='navCell'; else $ncss='navCellPgs';
}
foreach($rng as $r) {
if($r+PAGE1_OFFSET==$pars[0]) $c.=$pars[5].'<span class="'.$ncss.'">'.$r.'</span>';
else $c.=$pars[2].'<a style="text-decoration:none" class="'.$ncss.'" href="'.call_user_func('add'.$pars[3].'URLPage', $pars[1], $r+PAGE1_OFFSET).'" title="'.$GLOBALS['l_page'].' #'.$r.'">'.$r.'</a>';
}
return $c;
}
...
$nbspLim=' ';
if(isset($GLOBALS['l_mobilePreviousPage'])) $prevNav=$GLOBALS['l_mobilePreviousPage']; else $prevNav='&lArr;';
if(isset($GLOBALS['l_mobileNextPage'])) $nextNav=$GLOBALS['l_mobileNextPage']; else $nextNav='&rArr;';
}
else {
$sLim=11;$bLim=5;$cLim=4;
$nbspLim='&nbsp;&nbsp;';
//$prevNav='&laquo;&laquo;';
...
$pars=array($page, $url, $frn, $pageType, $navCell, $nbspLim);

if($spage-1>=1 and !$navCell) $pageNav.=$nbspLim.'<a href="'.call_user_func('add'.$pageType.'URLPage', $url, $page-1).'" title="'.$GLOBALS['l_page'].' #'.($spage-1).'">'.$prevNav.'</a>';
...
if($spage+1<=$pages and !$navCell) $pageNav.=$nbspLim.'<a href="'.call_user_func('add'.$pageType.'URLPage', $url, $page+1).'" title="'.$GLOBALS['l_page'].' #'.($spage+1).'">'.$nextNav.'</a>';

//$pageNav=$pN.'&nbsp;'.$GLOBALS['l_page'].' '.($page-PAGE1_OFFSET).' '.$GLOBALS['l_pageOf'].' '.$pages.':'.$pageNav;

if(!$navCell) {
if(!defined('NO_PAGE_ICON')) $pN=(defined('CUSTOM_PAGE_ICON')?CUSTOM_PAGE_ICON:'<img src="'.$GLOBALS['main_url'].'/img/page.gif" style="width:9px;height:10px" alt="'.$GLOBALS['l_page'].'" />&nbsp;'); else $pN='';
if(!defined('MOBILE')) $pageNav=$pN.'&nbsp;'.$GLOBALS['l_page'].' '.($page-PAGE1_OFFSET).' '.$GLOBALS['l_pageOf'].' '.$pages.':'.$pageNav;
else $pageNav=$pN.$pageNav;
}
}
return $pageNav;
}

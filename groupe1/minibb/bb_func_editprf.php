...
if(isset($_GET['adminUser'])) $adminUser=(int)$_GET['adminUser']+0; elseif(isset($_POST['adminUser'])) $adminUser=(int)$_POST['adminUser']+0; else $adminUser=0;
...
$deleteProfileLnk="{$l_sepr}&nbsp;<a href=\"{$main_url}/{$indexphp}action=removeuser&amp;user={$adminUser}\" target=\"_blank\">{$l_removeUser}</a>";
}
else $adminUser=0;
...
$l_passOnceAgain.='<br /><span class="txtSm">('.$l_onlyIfChangePwd.')</span>';
$l_sub_pass.='<br /><span class="txtSm">('.$l_onlyIfChangePwd.')</span>';
...
$tpl=preg_replace("#<!--INITMSG-->(.*?)<!--/INITMSG-->#is", '', $tpl);
if($adminUser==0 and $user_num_posts<$allowHyperlinksProfile) $tpl=preg_replace("#<!--WEBSITE-->(.*)<!--/WEBSITE-->#is",'',$tpl);

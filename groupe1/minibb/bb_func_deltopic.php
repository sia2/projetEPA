...
if(!in_array($poster_id, $pUsers) and $poster_id!=0) $pUsers[]=$poster_id;
...
db_calcTotalUserAmount($val, $Tu, $Tt, $Tp, $Taus, $arcId);
}
...
if($user_sort==1){
if(isset($metaLocation)) { $meta_relocate="{$main_url}/{$indexphp}action=vtopic&forum={$forum}&page={$return}&h={$h}"; echo ParseTpl(makeUp($metaLocation));  } else { header("{$rheader}{$main_url}/{$indexphp}action=vtopic&forum={$forum}&page={$return}&h={$h}"); }
exit;
}
else{
...
if(isset($metaLocation)) { $meta_relocate=$urlp; echo ParseTpl(makeUp($metaLocation)); } else { header("{$rheader}{$urlp}"); }
exit;
}

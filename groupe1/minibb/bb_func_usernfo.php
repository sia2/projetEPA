...
$activity=(isset($_GET['activity'])?(int)$_GET['activity']+0:-1);
if(!in_array($activity, array(0,1))) unset($activity);
...
if($canBlock and ($user_id==1 or $isMod==1) and $user!=1 and $user_id!=$user and isset($activity)){
$$dbUserAct=$activity;
updateArray(array($dbUserAct),$Tu,$dbUserId,$user);
//remove emails from subscriptions when making the user inactive
if($activity==0) db_delete($Ts, 'user_id', '=', $user);
}
...
$parseArc=str_replace('-', '_', ARCHIVE);

//$addFieldsGen[]=$dbUserSheme['num_live_topics'][1];
//$addFieldsGen[]=$dbUserSheme['num_live_posts'][1];

if($ress=db_simpleSelect(0, $Taus, 'num_topics, num_posts', 'user_id', '=', $user, '', '', 'archive_id', '=', $parseArc)) {

...
Latest File Update: 2015-Sep-27
*/
...
if(!defined('ARCHIVE')){
$tuW=(isset($statsDefField)?$statsDefField:'topic_last_post_time');
$tuE='>=';
$tuP=$timeLimit;
...
if($rww=db_simpleSelect(0,$Tu,'count(*)')) $numrs=$rww[0]; else $numrs=0;
$numUsers=parseStatsNum($numrs);
...
if($rww=db_simpleSelect(0,$Tt,'count(*)')) $numrs=$rww[0]; else $numrs=0;
$numTopics=parseStatsNum($numrs);

if($rww=db_simpleSelect(0,$Tp,'count(*)')) $numrs=$rww[0]; else $numrs=0;
$numPosts=parseStatsNum($numrs);
}

if($rww=db_simpleSelect(0, $Tu, $dbUserSheme['username'][1], $dbUserId, '=', 1)) $adminInf=$rww[0];
if($rww=db_simpleSelect(0, $Tu, "{$dbUserId}, {$dbUserSheme['username'][1]}", '', '', '', "{$dbUserId} DESC", 1)) $lastRegUsr=$rww;

...
//echo load_header();
//if($page==PAGE1_OFFSET+1 and (!isset($startPageModern) or !$startPageModern)) echo ParseTpl(makeUp('main_forums'));
if($page==PAGE1_OFFSET+1 and (!isset($startPageModern) or !$startPageModern)) $forumsTpl=ParseTpl(makeUp('main_forums')); else $forumsTpl='';
}
else{
$errorMSG=$l_stillNoForums; $l_returntoforums=''; $correctErr='';
//echo load_header(); echo ParseTpl(makeUp('main_warning'));
$forumsTpl=ParseTpl(makeUp('main_warning'));
}
?>
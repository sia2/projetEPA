...
$num_subvention = '';
$page_retour = "#";
$page_traite = 'confirmer-supprimer-subvention';
$id_jalon = '';


if( isset($_GET['sb']) and isset($_GET['pj'])) { 
	if (!empty($_GET['sb']) and !empty($_GET['pj'])) {
		$page_traite = 'confirmer-supprimer-subvention.php';
		$id_jalon = $_GET['sb'];
		$id_projet = $_GET['pj'];
		$page_retour = "subvention.php?sb=" . $id_jalon . '&pj=' . $id_projet;
	}
}
if( isset( $_POST['confirmer'])) { 
		...
		$sql = "DELETE FROM subvention WHERE id_subvention=" . $id_jalon; 

		//on insère les informations du formulaire dans la table 
		mysql_query($sql) or die('Erreur SQL !'.$sql.'<br>'.mysql_error()); 

		header('location: supprimer-subvention.php?pj=' . $id_projet ); 	  
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
		  <div class="row">
			...
			<div class="alert alert-success" role="alert">
					<a href="#" class="alert-link">Souhaitez vous supprimer la subvention ? </a>
				</div>
					<form method="POST" action="<?php echo $page_traite; ?>">
						<button type="submit" class="btn btn-default">Confirmer</button>
						<input type="hidden" name="sb" value="<?php echo $id_jalon; ?>">

...
if( isset($_GET['jl']) and isset($_GET['pj'])) { 
	if (!empty($_GET['jl']) and !empty($_GET['jl'])) {
		$page_traite = 'confirmer-archivage-jalon.php';
		$id_jalon = $_GET['jl'];
		$id_projet = $_GET['pj'];
		$page_retour = "jalon.php?jl=" . $id_jalon . '&pj=' . $id_projet;
	}
}
if( isset( $_POST['confirmer'])) { 
		...
		//$sql = "DELETE FROM jalon WHERE id_jalon=" . $id_jalon; 

		//on insère les informations du formulaire dans la table 
		mysql_query($sql) or die('Erreur SQL !'.$sql.'<br>'.mysql_error()); 

		header('location: archiver-jalon.php?pj=' . $id_projet ); 	  
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
		  <div class="row">
			...
			<div class="alert alert-success" role="alert">
					<a href="#" class="alert-link">Souhaitez vous archiver le jalon ? </a>
				</div>
					<form method="POST" action="<?php echo $page_traite; ?>">
						<button type="submit" class="btn btn-default">Confirmer</a></button>
